...
public class HeartOfIronUpgradable extends HeartOfIron implements UpgradablePotion {
    public static final String POTION_ID = testMod.DefaultMod.makeID(HeartOfIronUpgradable.class.getSimpleName());

    private static final PotionStrings potionStrings = CardCrawlGame.languagePack.getPotionString(POTION_ID);
    private static final PotionStrings oldPotionStrings = CardCrawlGame.languagePack.getPotionString(HeartOfIron.POTION_ID);
    ...
    public int getPotency(int ascensionLevel) {
        if (getPotionLevel() < maxPotionLevel)
            return 6 + (int) Math.floor(getPotionLevel() / 3);
        else
            return 10;
    }
